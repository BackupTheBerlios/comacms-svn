...
/*        function _start() {
        ...
        function replace_smilies($textdata) {
                $sql = "SELECT smilie_id, smilie_path, smilie_text, smilie_title FROM cms_smilies";
                $result = db_result($sql);

                while($smilie = mysql_fetch_object($result)){
                  $textdata = str_replace($smilie->smilie->text, "<img src=\"".$smilie->path."\" alt=\"".$smilie->title."\">", $textdata);
                }
                return $textdata;
        }
